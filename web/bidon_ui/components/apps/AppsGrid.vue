    ...
    <template #card="{ item: app, deleteHandle }">
      <div class="card flex flex-col">
        ...
        <div class="card-header gap-3">
          ...
          <span class="badge badge-platform shrink-0">{{
            app.platformId
          }}</span>
        </div>
        ...
        <div class="card-body flex-1 py-3 space-y-1.5">
          ...
          <div v-if="app.storeId" class="flex items-center gap-2">
            <i
              class="pi pi-tag text-xs shrink-0 w-3.5"
              style="color: var(--bidon-muted)"
            />
            <span
          ...
          <div
            class="flex items-center gap-1.5 pt-2 mt-1 border-t"
          ...
          >
            <span class="badge badge-count-rtb">
              ...
              {{ auctionConfigCount(app.id as number) }} Auction
              {{
                auctionConfigCount(app.id as number) === 1
                  ? "Config"
                  : "Configs"
              }}
            </span>
...
const { data: auctionConfigData } = useAsyncData(
  ...
  () =>
    $apiFetch<Collection<CountableItem>>(
      "/v2/auction_configurations_collection",
      {
        params: { limit: 10000 },
      },
    ),
...
const auctionConfigCount = (appId: number) =>
  auctionConfigCounts.value.get(appId) ?? 0;
