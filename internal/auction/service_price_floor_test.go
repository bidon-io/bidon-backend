...
func TestPriceFloor(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: decimal.RequireFromString("0.25"), // Should use previous auction price since it's higher than calculated floor
		},
		{
			...
			expected: decimal.RequireFromString("0.05"), // Should use calculated floor since it's higher than previous auction price
		},
		{
			...
			expected: decimal.RequireFromString("0.05"), // Should use max of request, cache, and config
		},
		{
			...
			expected: decimal.RequireFromString("0.05"), // Should use max of request, cache, and config (ignore previous auction price)
		},
		{
			...
			req: &schema.AuctionRequest{
				...
				AdCache: []schema.AdCacheObject{
					...
					{Price: decimal.RequireFromString("0.03")},
				},
			},
			auctionConfig: &Config{
				PriceFloor: decimal.RequireFromString("0.05"),
			},
			expected: decimal.RequireFromString("0.07"), // Should use max of request, highest cache, and config
		},
		{
			...
			expected: decimal.RequireFromString("0.1"), // Should use request price floor
		},
		{
			name: "Empty ad cache",
			req: &schema.AuctionRequest{
			...
			expected: decimal.RequireFromString("0.05"), // Should use config price floor
		},
		{
			...
			expected: decimal.RequireFromString("0.25"), // Should use previous auction price
		},
		{
			...
			expected: decimal.RequireFromString("0.05"), // Should use calculated floor since previous auction price is lower
		},
		{
			...
			expected: decimal.RequireFromString("0.0"), // Should return 0 (disabled floor) for specified auction key with custom adapter
		},
		{
		...
		},
		{
			...
			expected: decimal.RequireFromString("0.05"), // Should use normal floor logic since adapter is not custom
		},
		{
			...
			expected: decimal.RequireFromString("0.05"), // Should use normal floor logic since auction key is not in disabled list
		},
		{
			...
			expected: decimal.RequireFromString("0.05"), // Should use normal floor logic since auction key is empty
		},
	}
