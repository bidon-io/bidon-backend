...
// HandleStatsV2 is used to handle v2/stats request
// Finalize results of auction in redis
// If external_win_notification is enabled - do nothing, wait /win or /loss request
// If external_win_notification is disabled - send win/loss notifications to demands
func (h Handler) HandleStatsV2(ctx context.Context, stats schema.StatsV2, config *auction.Config, bundle, adType string) {
	if config == nil {
		log.Printf("HandleStatsV2: cannot find config: %v", stats.AuctionConfigurationID)
		return
	}
	...
	auctionResult, err := h.AuctionResultRepo.Find(ctx, stats.AuctionID)
	if err != nil {
		log.Printf("HandleStatsV2: AuctionResult exception: %s", err)
		return
	}

	if auctionResult == nil {
		log.Printf("HandleStatsV2: AuctionResult not found: %s", stats.AuctionID)
		return
	}
	...
	if len(prices) == 0 {
		log.Printf("HandleStatsV2: no valid prices: %s", stats.AuctionID)
		return
	} else if len(prices) == 1 {
		firstPrice = prices[0]
		secondPrice = prices[0]
